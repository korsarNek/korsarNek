...
const localeToTerritory = moize_1.default.shallow(str => {
...
const meta = (name, content, escape) => {
    if (escape !== false && typeof content === 'string') {
        content = (0, hexo_util_1.escapeHTML)(content);
    }
    if (content)
...
function openGraphHelper(options = {}) {
    ...
    const { content } = page;
    let images = options.image || options.images || page.image || page.photos || [];
    ...
    const url = (0, hexo_util_1.prettyUrls)(options.url || this.url, config.pretty_urls);
    ...
    if (!Array.isArray(images))
        images = [images];
    if (description) {
        description = (0, hexo_util_1.escapeHTML)((0, hexo_util_1.stripHTML)(description).substring(0, 200)
    ...
    if (url) {
        result += og('og:url', (0, hexo_util_1.encodeURL)(url), false);
    }
    else {
        result += og('og:url');
    }
    ...
    if (date) {
        if (((0, moment_1.isMoment)(date) || (0, moment_1.isDate)(date)) && !isNaN(date.valueOf())) {
            result += og('article:published_time', date.toISOString());
        }
    }
    if (updated) {
        if (((0, moment_1.isMoment)(updated) || (0, moment_1.isDate)(updated)) && !isNaN(updated.valueOf())) {
...
hexo.extend.helper.register('open_graph', openGraphHelper);
